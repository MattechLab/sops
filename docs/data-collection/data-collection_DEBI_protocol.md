...
<figure style="display: inline-block; text-align: center; margin-right: 10px;">
  <img src="../assets/debi_protocol/overview/overall_setting.png" alt="overall experiments" style="width: 80%;">
  <figcaption>Overall experimental setting</figcaption>
</figure>

The graph above can be divided into the following components:
1. Syncbox: A NordicLabs Syncbox sends TTL (transistor-transistor logic) triggers to the scanner and forward the signal converted to the keyboard signal "s" to the PsychoPy laptop. 
<figure style="display: inline-block; text-align: center; margin-right: 10px;">
  <img src="../assets/debi_protocol/selected/19-pc-trigger-signal.png" alt="Trigger to PsychoPy PC" style="width: 80%;">
  <figcaption>Trigger to PsychoPy PC</figcaption>
</figure>

<figure style="display: inline-block; text-align: center; margin-right: 10px;">
  <img src="../assets/debi_protocol/selected/19-scanner-signal.png" alt="Trigger to Scanner" style="width: 80%;">
  <figcaption>Trigger to Scanner</figcaption>
</figure>

2. Scanner: 3T clinical scanner (MAGNETOM PrismaFit, Siemens Healthineers) with a 64-channel head-neck coil with an attached mirror.
3. Eye tracker: We use the EyeLink 1000 Plus (SR Research Ltd.) for eye tracking.
...
### Collect ET data 
- Copy data from the subfolder of PsychoPy program into the hard disk.
...
- In the ICE terminal: ideacmdtool -> type `4` -> type `6` //
...
- The only useful thing is the physio [flag on External Signal] <mark> need more explanation </mark>
- Il faut pas oublier de faire save as total single raid files - pour sauvegardé les noises measurements. <mark> need more explanation </mark>
